...
import (
	...
	"github.com/agrovision/production-service/internal/handler"
...
func main() {
	...
	var repo *repository.ProductionRepository

	if !demoMode {
		repo = repository.NewProductionRepository(db)
	} else {
		repo = repository.NewProductionRepository(nil)
	}

	productionHandler := handler.NewProductionHandler(repo)
	...
	productionHandler.RegisterRoutes(router)

	port := getEnv("SERVICE_PORT", "8082")
	log.Printf("production-service listening on :%s", port)
